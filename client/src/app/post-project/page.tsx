  ...
  async function postProject() {
    ...
    try {
      const res = await apiFetch("/projects", {
        method: "POST",
        body: JSON.stringify({ client_id: user?.id, title, description: desc, budget_min: Number(budgetMin), budget_max: Number(budgetMax), skills, project_type: type, deadline }),
      }, token);
      if (res.ok) {
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-xs text-primary/80 leading-relaxed">
            Describe your project clearly — our AI will score incoming proposals for relevance, matching freelancers with the right skills automatically.
          </div>
